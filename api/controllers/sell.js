...
module.exports = async (req, res) => {
  ...
  if (!usersShare)
    return res.status(400).json({
      resultMessage: 'Your portfolio does not contain that share',
    });

  if (req.body.amounth > usersShare.amounth)
    return res.status(400).json({
      resultMessage: 'You should own more than you want to sell.',
    });
  ...
  if (transaction) {
    usersShare
      .update({
        amounth: usersShare.amounth - req.body.amounth,
      })
      .catch((err) => {
        return res.status(400).json({ err });
      });
    ...
    return res.status(200).json({
      resultMessage: 'Successfully selled',
    });
  }
};
